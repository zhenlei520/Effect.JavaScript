...
﻿; (function () {
    ...
    var AlloyTouch = function (option) {
        ...
        this.startTime;
        this.start;
        this.topDis = option.topDis;
        this.bottomDis = option.bottomDis;
        this.bottomMax = option.bottomMax;
            ...
            if (this.isTouchStart) {
                ...
                if (this._preventMoveDefault) {
                    ...
                    if (this.hasMax && this.scroller[this.property] > this.max && d > 0) {
                        this.factor1 = this.topDis;
                    } else if (this.hasMin && this.scroller[this.property] < this.min && d < 0) {
                        if ( this.scroller[this.property] < (this.min-this.bottomMax) ) {
                            this.scroller[this.property] = this.min-this.bottomMax;
                        }
                        this.factor1 = this.bottomDis;
                    } else {
                        this.factor1 = 1;
                    }
                    d *= this.factor1;
                ...
                if (this.hasMax && this.scroller[this.property] > this.max) {
                    ...
                    if (duration < 300) {
                        var distance = ((this.vertical ? evt.changedTouches[0].pageY : evt.changedTouches[0].pageX) - this.start) * this.sensitivity,
                            ...
                            destination = this.scroller[this.property] + (speed2 * speed2) / (4 * this.deceleration) * (distance < 0 ? -1 : 1);

                        self.to(this.scroller, this.property, Math.round(destination), Math.round(speed / self.deceleration /2), iosEase, function (value) {
